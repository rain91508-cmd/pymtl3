...
class Component( ComponentLevel7 ):
  ...
  #-----------------------------------------------------------------------
  # Inter-test state cleanup
  #-----------------------------------------------------------------------

  @classmethod
  def reset_test_state( cls ):
    """Reset PyMTL3 global state that accumulates across tests.

    Call this between tests (e.g. via a pytest autouse fixture) to prevent
    global state leakage from one test affecting the next.

    Cleans up:
      1. linecache.cache entries generated by GenDAGPass / SimpleSchedulePass
      2. Random seed for deterministic schedule ordering
      3. Stale greenlet references via garbage collection

    Without this, tests that pass in isolation may fail in a suite because:
      - Compiled net-block functions from a previous test shadow new compilations
      - random.shuffle in SimpleSchedulePass produces different orderings
      - WrapGreenletPass greenlets hold stale references to old components
    """
    import linecache
    import gc

    # 1. Clear PyMTL3-generated linecache entries (from GenDAGPass, etc.)
    pymtl3_keys = [
        k for k in linecache.cache
        if k.startswith('Net (writer') or k == 'ff_flips'
          or 'Update Block' in k
    ]
    for k in pymtl3_keys:
        try:
            del linecache.cache[k]
        except KeyError:
            pass

    # 2. Reset random seed for deterministic schedule ordering
    import random
    random.seed(42)

    # 3. Force garbage collection to release stale greenlet references
    gc.collect()
